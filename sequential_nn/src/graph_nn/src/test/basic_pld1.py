...
if 'edge_label' in edge_storage:
    ratings = edge_storage.edge_label
    print("評価ラベル: edge_label")
elif 'edge_attr' in edge_storage:
    ratings = edge_storage.edge_attr
    print("評価ラベル: edge_attr")
else:
    # それ以外の場合は最初の属性を仮定（通常は 'y' や 'rating' など）
    key = list(edge_storage.keys())[0]
    ratings = edge_storage[key]
    print(f"評価ラベル: {key}")
