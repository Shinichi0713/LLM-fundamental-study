...
__1. 計算効率の大幅改善__
- **平均解像度処理量（≒計算量）を最大80%削減**  
  CARES を挟むことで、ターゲットVLMが処理する画像解像度（および対応する計算量）を大幅に削減できた[arXiv](https://arxiv.org/abs/2510.19496)[GitHub](https://github.com/mkimhi/CARES)。
- これは、**視覚トークンが入力の97〜99%を占める**という事実を踏まえると、非常に大きな改善です。

__2. タスク性能のほぼ維持__
- **質問回答精度をフル解像度ベースラインの1%以内に維持**  
  CARES を挟んだ場合でも、QA精度がフル解像度で処理した場合と比べて**1%以内の誤差に収まる**ことが示されている[GitHub](https://github.com/mkimhi/CARES)。
- つまり、**計算コストを大幅に削減しつつ、タスク性能はほぼ変わらない**という結果です。

__3. ベンチマーク・ターゲットVLMにわたる一貫性__
- **文書系（DocVQA, ChartQA, InfographicVQA）** と**自然画像系（TextVQA, HME100K）** の両方で、同様の傾向が確認されている[GitHub](https://github.com/mkimhi/CARES)。
- **SmolVLM系**と**IBM Granite-Docling**といった異なるターゲットVLMに対しても、  
  **計算削減と性能維持**が一貫して確認されており、  
  CARES が**汎用的な前処理モジュール**として機能することが示されています。

__4. 実験結果の位置づけ__
- 論文の結果は、「**精度で他手法を大きく上回る**」というより、  
  **「精度をほぼ維持したまま、計算コストを大幅に削減できる」** という**効率改善型**の成果です。
- そのため、図表では「Oursがすべての指標で圧倒」というより、  
  **計算量削減と精度維持のトレードオフ**が強調されています。

### 一言でまとめると

CARES は、**「画像＋質問に応じて必要な解像度を選ぶ」** ことで、**計算量を最大80%削減しつつ、QA精度をほぼ維持できる**ことを、複数のベンチマークとターゲットVLMで示した研究でした[arXiv](https://arxiv.org/abs/2510.19496)[GitHub](https://github.com/mkimhi/CARES)。

途中までは精度上がるんじゃないかと思っていたら、計算コストを抑えるという結論だったという。。。
そう簡単にうまくはいかないか・・・。
