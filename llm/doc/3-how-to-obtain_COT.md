LLM（大規模言語モデル）が、単なる言語パターン復唱機から、論理的な推論や複雑な問題解決を行う「思考力」を獲得するまでの学習の流れは、3つの段階を経て進展します。
この「思考力」は、特に**Chain-of-Thought (CoT)** の能力、つまり**段階的な推論プロセス**を生成する能力を指すことが多いです。

といった辺りでLLMが**人のような思考力**(のようなもの)を得るまでの学習について、本日は説明していきます。
...
__ステップ 1: 報酬モデル (Reward Model, RM) の学習__
...
したがって、命令チューニングは「何を答えるべきか」の**知識と形式**を教える手法ですが、「どのように答えるべきか」の**品質と多様性**を評価し、学習するのは**報酬モデル**を用いたRLHFの役割となります。 

## 5. RLHF
先ほどの文章の続きとして、「多様性がなぜ必要か」からRLHFの仕組みを整理すると、次のような流れになります。

### なぜ「多様性」が必要なのか

LLMの応答には、**単一の「正解の言い回し」が存在しない**場面が非常に多いです。

- 同じ質問に対しても、  
  - ユーザーの知識レベル（専門家か、初心者か）  
  - 場面（ビジネス文書か、カジュアルなチャットか）  
  - 文化・言語の違い  
によって、適切な表現は変わります。

- また、思考プロセス（CoT）を書かせる場合でも、  
  - 箇条書きで整理する  
  - 段落形式で説明する  
  - 例示を多く入れる  
など、**表現スタイルは多様であってよい**はずです。

ところが、命令チューニング（SFT）のクロスエントロピー損失は、  
「データセットに書かれている特定のトークン列」以外を強くペナルティします。  
その結果、モデルは**意味的に正しくても、言い回しが違う応答を「悪い出力」として学習**してしまいます。

このままでは、  
- ユーザーが求めるスタイルに柔軟に対応できない  
- 創造的な言い換えや、より自然な表現が生まれにくい  
- CoTの書き方も画一的になり、説明の柔軟性が失われる  

という問題が起きます。  
そのため、**「意味が同じなら、表現は多様であってよい」という多様性**を確保することが重要になります。

### RLHFの仕組みの概要（多様性確保の観点から）

RLHF（Reinforcement Learning from Human Feedback）は、  
「特定の言い回しに縛られず、**抽象的な品質**を評価して学習する」仕組みです。

__1. 報酬モデル（Reward Model, RM）の学習__

まず、**「どの応答が良いか」を判断する評価関数**を別途学習します。

- 手順の概要  
  1. SFT済みのLLMに同じ指示を与え、複数の応答（A, B, C, …）を生成させる。  
  2. 人間がそれらを比較し、「どれが一番良いか」「どれが次に良いか」といった**順位付け（Preference）** を行う。  
  3. その順位付けデータを使って、**報酬モデル（RM）**という小さなモデルを教師あり学習で訓練する。  
  4. RMは、指示 $I$ と応答 $R$ のペアを入力として、その応答の**品質スコア $R(I, R)$** を出力するように学習される。

- 多様性との関係  
  - RMは「特定のトークン列」ではなく、**人間が感じる抽象的な品質**（有用性、無害性、論理性、自然さなど）を評価する。  
  - そのため、**言い回しが違っても、意味が良ければ高スコア**になる。  
  - これにより、「データセットに書かれていない表現」でも、人間が好むなら報酬が得られるようになる。

__2. 強化学習（RL）によるLLMの調整__

次に、学習した報酬モデルを使って、LLM自体を強化学習で微調整します。

- 手順の概要  
  1. LLMが指示 $I$ に対して応答 $R$ を生成する（これが「行動」に相当）。  
  2. 生成された応答 $R$ をRMに入力し、**報酬 $r = R(I, R)$** を得る。  
  3. LLMは、この報酬を最大化するようにパラメータを更新する（PPOなどのRLアルゴリズムを使用）。  

- 多様性との関係  
  - LLMは、**クロスエントロピー損失に縛られず**、RMが高く評価する応答を出す方向に学習される。  
  - RMが「多様な言い回しでも、意味が良ければ高評価」するため、LLMも**多様な表現で高品質な応答**を生成するようになる。  
  - これにより、SFT段階で起きていた「単一の言い回しへの過剰なバイアス」が緩和される。

## 総括

総括すると、LLMが「人のような思考力」を得るまでの流れは、大きく次の2つの側面から整理できます。

### 1. 「思考力」獲得の3段階

1. **事前学習（Pre-training）**  
   - 大量のテキストから「次の単語予測」を通じて、言語の統計的パターンと世界の知識を獲得。  
   - 知識の基盤はできるが、**自発的な論理推論**はまだ弱い。

2. **命令チューニング（SFT）**  
   - 指示と回答のペアで微調整し、「質問に答える」「要約する」など**タスクの形式**を学習。  
   - 「ステップバイステップで考えなさい」といったCoTプロンプトに従う**形式**を学ぶが、推論の質はまだ限定的。

3. **推論能力の獲得と洗練（CoTチューニング＋RLHF/DPO）**  
   - CoTデータセットで**段階的な推論プロセス**を直接学習させ、Few-shot/Zero-shot CoTの基盤を作る。  
   - RLHF/DPOで、人間の好みに沿った**論理的・倫理的な推論**を強化し、思考プロセスを人間に理解しやすく、エラーが少ない形に洗練する。

この3段階を通じて、  
「次の単語予測」という基本動作を保ちつつ、**次に予測される単語が“論理推論の次のステップ”になるように**学習を誘導することで、LLMは人のような思考力を獲得していきます。

### 2. 命令チューニングの限界とRLHFによる多様性確保

- **SFTの限界**  
  - クロスエントロピー損失は「データセットに書かれた特定の言い回し」以外を強くペナルティする。  
  - そのため、**意味が同じでも言い回しが違う応答を“悪い出力”として学習**し、表現の多様性が失われやすい。

- **RLHFの役割**  
  - 報酬モデル（RM）が、特定の単語列ではなく、**人間が感じる抽象的な品質**（有用性・自然さ・論理性など）を評価する。  
  - 強化学習でLLMを調整し、**多様な言い回しでも高品質な応答**を出すように誘導する。  
  - これにより、SFTで起きていた「単一の言い回しへの過剰なバイアス」を緩和し、**柔軟で多様な思考表現**を可能にする。
