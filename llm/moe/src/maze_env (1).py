...
class MazeEnv:
    ...
    def __init__(self, maze_file="maze.txt", rows=5, cols=5):
        ...
        # --- 修正: 往復移動（同じ2マスを行き来する）でも合計プラスになる抜け穴を修正 ---
        # 近づく1歩: step_penalty + progress_reward = -0.05 + 0.3  = +0.25
        # 遠ざかる1歩(旧): step_penalty + regress_penalty(旧-0.15) = -0.05 - 0.15 = -0.20
        # -> 1往復(2手)で +0.25 -0.20 = +0.05 と、ゴールに向かわず往復するだけで
        #    確実にプラス収支になってしまっていた（往復ファーミングの抜け穴）。
        # 1往復の合計が0以下になるよう regress_penalty を強化する:
        #   progress_reward + regress_penalty + 2*step_penalty <= 0
        #   0.3 + regress_penalty - 0.1 <= 0  ->  regress_penalty <= -0.2
        # マージンを持たせて -0.25 に設定（1往復で -0.05 の確実な赤字にする）。
        self.regress_penalty = -0.25     # ゴールから遠ざかった場合
    ...
    def reset(self, maze_change=True, min_distance=None, max_distance=None, maze_override=None):
        """
        maze_override: (maze, start, goal) のタプルを渡すと、ランダム生成を
        スキップしてその迷路をそのまま使う。苦手迷路プール(MazePool)から
        取り出した迷路を出題する際に使う。
        """
        if maze_override is not None:
            maze, start, goal = maze_override
