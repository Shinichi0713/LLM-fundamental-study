...
class TransformerPPOAgent:
    ...
    def train(
        ...
        episode_rewards = []
        success_flags = []  # 直近のゴール到達可否を記録（診断用）
        ...
        for episode in range(num_episodes):
            ...
            for step in range(max_steps_per_episode):
                ...
                # 修正: horizon到達 or done(=ゴール到達)の場合はここで更新。
                # timeout（doneにならずstepが尽きるケース）はこの中では拾えない。
            ...
            # -------------------------------------------------------------
            # 【重要な修正】エピソードがtimeoutで終わった場合（doneがFalseのまま
            # ループを抜けた場合）、上のループ内ではupdate()が呼ばれない。
            # これを放置すると、bufferにこのエピソードの遷移が残ったまま
            # 次エピソード（＝全く別の迷路・別のスタート地点）のデータが
            # 同じbufferに連続して追記されてしまう。
            # dones=Falseのままなので、GAE計算がエピソード境界をまたいで
            # 無関係な軌跡を「連続した1本の軌跡」として扱ってしまい、
            # 価値関数・Advantageの推定が破壊される。
            # -> エピソードが尽きた時点で必ずupdate()を呼び、bufferを
            #    毎エピソードでクリアすることで、迷路をまたいだ汚染を防ぐ。
