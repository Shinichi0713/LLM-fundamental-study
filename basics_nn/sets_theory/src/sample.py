...
Original file is located at
    https://colab.research.google.com/drive/1M6vw09_7jvmVWGAX3Un_GwV6-KMmCFlS
"""
...
def plot_integer_ring_tables(n=5):
    """
    整数環 ℤ の有限部分 {-n,...,0,...,n} の加法・乗法の演算表を可視化
    """
    values = list(range(-n, n+1))
    size = len(values)
    ...
    # 加法表のプロット
    im0 = axes[0].imshow(add_table, cmap='coolwarm', interpolation='nearest')
    axes[0].set_title(f'Addition Table (Integers ℤ, Range [{-n},{n}])', fontsize=14)
    ...
    # 乗法表のプロット
    im1 = axes[1].imshow(mul_table, cmap='coolwarm', interpolation='nearest')
    axes[1].set_title(f'Multiplication Table (Integers ℤ, Range [{-n},{n}])', fontsize=14)
...
# 実行例
plot_integer_ring_tables(n=3)

def plot_zn_ring(n=6):
    """
    整数 mod n の環 ℤ/nℤ の加法・乗法表と零因子を可視化
    """
    values = list(range(n))
    size = n
    ...
    # 零因子のチェック（a ≠ 0, b ≠ 0 で a*b = 0 mod n）
    zero_divisors = []
    for a in range(1, n):
        for b in range(1, n):
            if (a * b) % n == 0:
    ...
    axes[0].set_title(f'加法表 ℤ/{n}ℤ', fontsize=14)
    axes[0].set_xticks(range(size))
    axes[0].set_yticks(range(size))
    axes[0].set_xticklabels(values)
    axes[0].set_yticklabels(values)
    ...
    axes[1].set_title(f'乗法表 ℤ/{n}ℤ (零因子を強調)', fontsize=14)
    axes[1].set_xticks(range(size))
    axes[1].set_yticks(range(size))
    axes[1].set_xticklabels(values)
    axes[1].set_yticklabels(values)
    ...
    for (a, b) in zero_divisors:
        axes[1].plot(b, a, 'ro', markersize=8, markeredgecolor='white')
    ...
    # 零因子の情報をテキストで表示
    if zero_divisors:
        print(f"ℤ/{n}ℤ の零因子の例:")
        for a, b in zero_divisors[:5]:  # 最初の5組だけ表示
            print(f"  {a} × {b} ≡ 0 (mod {n})")
    else:
        print(f"ℤ/{n}ℤ には零因子がありません（n は素数）")

# 実行例
plot_zn_ring(n=6)  # 合成数の例（零因子あり）
plot_zn_ring(n=5)  # 素数の例（零因子なし → 体）
