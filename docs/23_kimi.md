...
中国がリリースするLLMはアメリカ本土産のLLMを凌駕する勢いがあります。
彼らはアメリカ本土では入手可能な、高性能なGPUをさほど使うことが出来ません。
しかし、低コストで性能はほぼ同程度と考えられる高い性能を持っています。
本日はそんな中国産LLMの一つ Kimi について説明していきます。
...
![1788047713408](image/23_kimi/1788047713408.png)
...
まず結論ですが、**Kimi K3は「高性能GPUを使わずにリリースされた」わけではなく、むしろ輸出規制下のNVIDIA高性能GPUに大規模にアクセスできたことが、実現の核心だと報じられています。** ただし、2.8兆パラメータという巨大モデルでありながら「推論コストを大幅に抑えられた」のは、モデル設計の計算効率化によるものです。

### 1. ハードウェアへのアクセス（規制迂回が報じられている）

複数の報道機関（Bloomberg、CNBC等）によれば、Moonshot AIはKimi K3の学習にあたり、**Alibaba Cloudを通じて約2万台のNVIDIA Hopper/Blackwell系GPUクラスター**にアクセスしたとされています。<source-chip title="CNBC" url="https://www.cnbc.com/2026/07/23/moonshot-kimi-nvidia-ai-chips-export-ban.html" /><source-chip title="FourWeekMBA" url="https://fourweekmba.com/ai-moonshot-kimi-k3-alibaba-compute-loop-export-controls/" /> これに対し、ホワイトハウス高官は米国の輸出規制対象チップへの違法アクセスを非難しており、タイなど第三国のデータセンターを経由した規制迂回の可能性も指摘されています。<source-chip title="Asia Tech Review" url="https://www.asiatechreview.com/p/alibaba-moonshot-and-the-us-government" />

つまり、「高性能GPUを使わずに済んだ」のではなく、**規制の網目をくぐって高性能GPUを確保した**、あるいはクラウド経由で海外のGPUリソースにアクセスした、というのが実情とされています。

### 2. なぜ「少ないGPU」という印象があるのか：計算効率の革新

一方で、2.8兆パラメータという規模でありながら「Claude Fable 5に対して4.6分の1のコストでコーディングタスクをこなせる」<source-chip title="AlphaSignal" url="https://alphasignal.ai/news/moonshot-ai-s-kimi-k3-beats-claude-at-coding-for-4-6x-less-money" /> などと報じられている背景には、**モデル設計による計算量の劇的な削減**があります。

| 技術 | 効率化の仕組み |
|---|---|
| **Stable LatentMoE** | 896エキスパートのうちトークンごとに16個だけを活性化。総パラメータ2.8Tに対し、推論時の有効化パラメータは104Bに抑制。 |
| **KDA (Kimi Delta Attention)** | KVキャッシュを差分圧縮し、長文デコード時のメモリを約75%削減。 |
| **AttnRes** | 超深層での学習収束を高速化し、トレーニング効率を25%向上。 |

このため、**「同じGPUあたりの学習・推論効率」が極めて高い**ため、同等性能の密なモデル（Dense Model）と比較すれば「はるかに少ない実効計算量」で済んでいる、という意味では「高性能GPUをさほど使わずに済んだ」という印象が生じるのです。


## 総括

今回の内容からKimi K3の本質を以下にエッセンスとしてまとめました。

### 1. モデルの位置づけ
Moonshot AIが2026年7月に公開した、**世界初のオープン3Tクラス（総パラメータ2.8兆）マルチモーダル・エージェンティックモデル**です。有効化パラメータは104Bと推論コストを抑えつつ、テキスト・画像・動画を統合的に処理し、100万トークンの長文にも対応します。

### 2. 3つの技術的革新
巨大化による計算・メモリ・学習のボトルネックを、以下の3技術で同時に解消しています。

- **Kimi Delta Attention（KDA）**: KVキャッシュを「差分」で圧縮し、長文処理のメモリを約75%削減、デコード速度を6.3倍高速化
- **Attention Residuals（AttnRes）**: 超深層での情報伝達を「選択的索引」で最適化し、学習効率を25%向上
- **Stable LatentMoE**: 896エキスパートから16個を動的選択するスパース構造を、負荷分散と安定化技術で実用的に動作させ、スケーリング効率を約2.5倍に高める

### 3. 性能と強み
コーディング（SWE-Marathon等）、OS操作エージェント、長文処理、動画理解で最先端クラスのベンチマークを記録しています。特に24時間〜48時間の長期自律タスク（GPUカーネル最適化、コンパイラ開発、チップ設計、科学研究の再現）で高い実用性を示しています。

### 4. 限界と注意点
第三者評価では、Claude Fable 5やGPT-5.6 Solと比較して総合的に見劣りする場面や、幻覚率がやや高いという指摘もあります。また、ベンチマークスコアは使用する実行環境（ハーネス）に大きく依存するため、純粋なモデル間比較には注意が必要です。

**結論として、Kimi K3は「巨大モデルの実用化」という課題に対し、長文・深層・大規模MoEの3つの技術で革新的な解を示した、オープンソースコミュニティにとって重要な里程碑（マイルストーン）です。**
