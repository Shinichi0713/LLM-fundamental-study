...
![1788647634381](image/25_causal_GRPO_works/1788647634381.png)
...
本論文では、理論的主張を裏付けるために、**大きく2つの実験パート**が実施されています。以下にまとめます。
...
| パート | 目的 | 対応する理論的主張 |
|--------|------|------------------|
| **1. 勾配評価実験** | GRPOのPolicy GradientのMSE（平均二乗乗誤差）を測定し、Oracle推定量との比較 | Oracle Property、Optimality |
| **2. ポリシー最適化実験** | 実際にGRPOで学習し、グループサイズと性能の関係を検証 | Universal Scaling Law |

### 実験1：勾配評価実験（Gradient Evaluation）

__目的__

GRPOのPolicy Gradient推定量が、理論的に予測された通りに**Oracle推定量（真の価値関数を知っている理想的な推定量）に近づくか**を実証します。

__使用モデル__
- **Qwen/Qwen2.5-0.5B-Instruct**

__使用データセット__
- **Kyleyee/arithmetic-few-shot**: ICL（In-Context Learning）モデルの勾配評価用
- **Kyleyee/arithmetic-test**: BaseモデルおよびInstructモデルの勾配評価用

__実験手順__
1. **Oracle Vの計算**: 各プロンプトに対して、Monte Carloロールアウトで**1,000回**サンプリングし、真の期待報酬（Oracle価値関数）を近似
2. **3つの推定量のMSE比較**:
   - **Vanilla REINFORCE**: ベースラインなしの勾配推定
   - **GRPO**: グループ平均をベースラインとして使用
   - **Oracle**: 真の価値関数をベースラインとして使用

__検証結果（Figure 4）__
- グループサイズGを増やすと、**GRPOのMSEがOracleのMSEに漸近的に収束**
- Vanilla REINFORCEはGを増やしても改善が限定的
- これにより「GRPOはValue ModelなしでOracleと同等の性能に到達できる」という**Oracle Property**が実証されました

### 実験2：ポリシー最適化実験（Policy Optimization）

__目的__

理論的に導出した**最適グループサイズのスケーリング則**が、実際のLLM学習でも成立するかを検証します。

__使用モデル__
- **Qwen/Qwen2.5-Math-7B**（数学推論タスク向けモデル）

__使用フレームワーク__
- **VERL**（Volcengine Erlang RLフレームワーク）

__実験内容__
1. **グループサイズGを変化させた学習**:
   - G = 2, 4, 8, 16, 32, 64 など、様々なグループサイズでGRPO学習を実行
   - 固定された総サンプリング予算の中で、GとバッチサイズBのトレードオフを検証

2. **ベンチマーク評価**:
   - **GSM8K**（小学算数の文章題データセット）で精度を評価
   - 最適なGが存在することを実証（Figure 3, Figure 5）

__検証結果__
- **最適なグループサイズG*が存在**: Gが小さすぎると分散が大きく、大きすぎると1サンプルあたりのプロンプト数（B）が減り、性能が低下
- **スケーリング則の普遍性**: 最適なGは訓練予算やイテレーション数によらず、**データセットとモデルアーキテクチャのみで決定**
- 理論式 G* = √(c₃/c₁) が実験的に確認された

### 実験で使用された主要なデータセットとモデルまとめ

| 項目 | 勾配評価実験 | ポリシー最適化実験 |
|------|------------|------------------|
| **モデル** | Qwen2.5-0.5B-Instruct | Qwen2.5-Math-7B |
| **データセット** | arithmetic-few-shot, arithmetic-test | GSM8K |
| **GPU要件** | 1 GPU（48GB+ VRAM） | 4+ GPUs（各96GB+ VRAM） |
| **フレームワーク** | 独自Pythonスクリプト | VERL |

### 実験が裏付けた理論的主張

| 理論的主張 | 実験による証拠 |
|-----------|-------------|
| **GRPOはU-statisticである** | グループ内比較構造がMSEのO(1/G)収束を示す |
| **Oracle Property** | Gを増やすとGRPOのMSEがOracleのMSEに一致 |
| **Optimality** | GRPOがVanilla REINFORCEを一貫して上回る |
| **Universal Scaling Law** | GSM8Kで最適Gが理論予測と一致、かつ訓練予算に依存しない |

## 総括

この論文の本質は、**「GRPOのグループ平均は、単なる“安い代用品”ではなく、統計学的に最適な推定方法である」** という一点に尽きます。

より具体的には、以下の3つの核心を示しています：


### 1. GRPOのPolicy Gradientは「U-statistic」という統計学的に最適な構造を持つ
同じプロンプトに対する複数回答の**相対比較**から勾配を推定するというGRPOの仕組みは、1948年に提唱された統計学の「U-statistic」と完全に一致します。これにより、グループ平均によるCritic近似に厳密な数学的正当性が与えられました。

### 2. Value Modelなしで「神の視点（Oracle）」と同等の性能に到達できる
GRPOは、真の価値関数を完備した理想的なアルゴリズムと**漸近的に等価**です。つまり、重いValue Modelを別途学習しなくても、十分なグループサイズを確保すれば「本物のCriticを持っている時と同じ理論限界」に到達できます。

### 3. 最適なグループサイズGは訓練予算ではなく「タスクの性質」で決まる
最適なGはモデルやデータの性質のみで決まり、**訓練時間や計算コストには依存しません**。一度決めれば固定できる「普遍的スケーリング則」が存在します。

### 一言でまとめると：
> **「GRPOはValue Modelを省略した“妥協案”ではなく、統計学的に最適化された“本物のアルゴリズム”である」**

これが、DeepSeek-R1などでGRPOが驚異的な効率を発揮する理由の理論的解明です。
