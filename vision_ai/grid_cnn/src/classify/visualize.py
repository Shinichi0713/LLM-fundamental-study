...
def visualize_input_output():
    ...
    axes[0].set_title("入力画像（PIL形式）")
    axes[0].axis("off")

    # 右：テンソルを画像に戻したもの（参考）
    img_tensor_np = img_tensor.squeeze().cpu().numpy()  # (H, W)
    axes[1].imshow(img_tensor_np, cmap="gray")
    axes[1].set_title("モデル入力（テンソル → numpy）")
    axes[1].axis("off")

    plt.suptitle(
        f"正解: ({row_true}, {col_true}) | 予測: ({row_pred}, {col_pred}) | 一致: {'○' if (row_true, col_true) == (row_pred, col_pred) else '×'}"
    )
    plt.tight_layout()
    plt.show()
