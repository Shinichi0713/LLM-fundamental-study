...
KLダイバージェンスがあることで、潜在空間が**滑らかな連続空間**になります。これにより、潜在空間の2点間を補間（中間を取る）すると、画像も自然に変化します。
...
```
通常のオートエンコーダの潜在空間：
  訓練画像Aのベクトル ●        訓練画像Bのベクトル ●
                          ？？（何もない）
                          
  AとBの中間のベクトルをデコーダに入れると
  → 「意味不明なノイズ」が出てくる
```
...
潜在変数を**確率分布**（平均μと分散σ）として表現し、**KLダイバージェンス**で潜在空間全体を標準正規分布に近づけます。
...
z ~ q(z|x)   ← ランダムにサンプリング
     ↓
この「サンプリング」操作は微分できない！
→ 勾配が計算できない → 誤差逆伝播できない → 学習できない
```

__VAEの解決策：再パラメータ化トリック__

```
z = μ + σ × ε    （ε は標準正規分布からのランダム値）
...
従来の手法では、以下の2つを同時に満たすのが難しかったのです。

| 手法 | 表現学習（特徴抽出） | 生成（新しいデータ作成） |
|------|-------------------|----------------------|
| 通常のオートエンコーダ | ✅ 得意 | ❌ できない |
| 変分推論（統計学） | ✅ 理論的に可能 | ❌ 計算が複雑 |
| GAN | ⚠️ 間接的 | ✅ 得意 |
