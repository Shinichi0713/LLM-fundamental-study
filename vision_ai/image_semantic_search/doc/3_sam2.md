
物体検知やセグメンテーションモデルに慣れていた状態でSAM2(2026年8月時点の最新はSAM3ですが)を触ると非常に衝撃を受けます。
指定にそって検知対象を切り替えることが可能、精度も高い。
内容がどうなっているのか気になりました。

本日テーマ：
>SAM2について調べてみる

## SAM2とは

SAM2（Segment Anything Model v2）は、Meta（旧Facebook）が開発した**プロンプト駆動型の汎用セグメンテーションモデル**で、画像や動画中の任意のオブジェクトを高精度にマスク分割できる技術です。

### 1. 発表年・論文

- **発表年**：2024年  
- **論文**：  
  - タイトル：**SAM2: Segment Anything Model v2**  
  - 著者：Metaの研究チーム（Alexander Kirillov 氏ら）  
  - 公開：arXiv 2024年（プレプリント）  
  - URL：https://arxiv.org/html/2505.23769v1（正確なIDは公開版により異なります）

### 2. SAM2の特徴

__(1) プロンプト駆動のセグメンテーション__

- ユーザーが**点・ボックス・テキスト・マスク**などのプロンプトを与えると、それに基づいて画像中の対象オブジェクトをマスク分割します。  
- 例：  
  - 「この点が示す物体をセグメントして」  
  - 「このボックス内の物体をマスクして」  
  - 「“猫”というテキストでセグメントして」

__(2) 画像・動画対応__

- SAM2は**静止画だけでなく動画**にも対応しており、時間方向の一貫性を考慮したセグメンテーションが可能です。  
- 動画中のオブジェクトをフレーム間で追跡しながらマスクを生成できます。

__(3) 高精度な境界情報__

- SAM2は**Hiera**という階層型ビジョントランスフォーマーをバックボーンに採用し、  
  細かい境界や複雑な形状のオブジェクトも高精度にマスクできます。

__(4) ゼロショット・オープンワールド__

- 特定のカテゴリに限定されず、**任意のオブジェクト**をセグメントできる「ゼロショット」能力を持ちます。  
- 学習データに含まれない未知の物体でも、プロンプトに応じてマスクを生成できます。

![1785704008641](image/3_sam2/1785704008641.png)

### 3. SAM2の技術的なポイント

- **バックボーン**：Hiera（階層型ViT）  
- **プロンプトエンコーダ**：点・ボックス・テキスト・マスクなどを統一的に扱うエンコーダ  
- **マスクデコーダ**：プロンプト情報と画像特徴を統合してマスクを生成  
- **動画対応**：時間方向のアテンションやトラッキング機構により、動画中のオブジェクトを一貫してセグメント

### 4. 関連研究との関係

- **SAM（初代）**：2023年発表。画像のみ対応、ViTバックボーン。  
- **SAM2**：2024年発表。動画対応、Hieraバックボーン、より高精度・高速。  
- **MaTIR / TextRegion**：SAM2で得たマスクを、Alpha-CLIPやCLIPと組み合わせて**領域レベルの検索・理解**に応用する研究[MaTIR](https://arxiv.org/pdf/2506.22864)[TextRegion](https://arxiv.org/html/2505.23769v1)。

### 5. まとめ

- **SAM2**は、2024年にMetaが発表した**プロンプト駆動の汎用セグメンテーションモデル**で、  
  画像・動画中の任意オブジェクトを高精度にマスク分割できます。  
- SAM2で得たマスクをAlpha-CLIPやCLIP系モデルと組み合わせることで、  
  「画像内の特定領域をクエリとしてテキストを検索する」といった**領域レベルのImage-to-Text Retrieval**が実現可能です。

## モチベーション

SAM2の開発背景（「なぜ上記の課題を解決したいと考えたか」）は、主に**実世界の多様なシーンに対応するための汎用AI基盤**を作りたい、という大きな目標から来ています。

### 1. 実世界の多様さに対応するため

- 現実世界には、  
  - 既知のカテゴリ（人・車・犬など）だけでなく、  
  - 未知の物体、複雑な形状、多様な背景  
  が無数に存在します。
- 従来の「特定カテゴリ専用モデル」では、  
  - 新しい物体が出てくるたびに再学習が必要  
  - 複雑なシーンでは誤検出や精度低下が起きる  
  という限界がありました。
- SAM2は、**どんな物体でもプロンプト一つでセグメントできる汎用基盤**を目指し、  
  実世界の多様さに柔軟に対応できるように設計されています。

### 2. 動画・時系列データの重要性

- 現代のAI応用では、  
  - 監視カメラ映像  
  - 自動運転のセンサーデータ  
  - AR/VR、ロボティクス  
  など、**動画や時系列データ**が非常に重要になっています。
- しかし、動画では  
  - フレームごとに独立に処理すると、オブジェクトのIDが変わったり、マスクが揺れたりする  
  - トラッキングとセグメンテーションを別々に扱うと処理が重く、リアルタイム性が損なわれる  
  という課題がありました。
- SAM2は、**動画全体を一貫して扱うセグメンテーション**を実現し、  
  動画ベースの応用（追跡、行動認識、ARなど）を支える基盤を目指しています。

### 3. 高精度な境界情報の需要

- 多くの応用（画像編集、医療画像、ロボットビジョンなど）では、  
  - 物体の**輪郭や境界**が非常に重要  
  - 少しの誤差が結果に大きく影響する  
  という特徴があります。
- 従来モデルでは、  
  - 単純な形状は得意だが、細かい境界や複雑な形状では精度が落ちる  
  という課題がありました。
- SAM2は、**Hieraバックボーンとプロンプト駆動**により、  
  - 細かい境界  
  - 複雑な形状（髪の毛、葉っぱ、網目など）  
  も高精度にマスクできるように設計されています。

### 4. 汎用AI基盤としての役割

- SAM2は単なる「セグメンテーションツール」ではなく、  
  - MaTIR（Mask-aware Text-to-Image Retrieval）  
  - TextRegion（テキストと整合した領域トークン）  
  など、**より高度なマルチモーダルタスクの基盤**としても活用されています[MaTIR](https://arxiv.org/pdf/2506.22864)[TextRegion](https://arxiv.org/html/2505.23769v1)。
- 「任意の物体を高精度にマスクできる基盤」があれば、  
  - 画像検索  
  - 参照表現理解  
  - ロボットの物体操作  
  - 画像編集・AR  
  など、さまざまな応用に横展開できます。

## 特徴

SAM2の一番の特徴は、**「プロンプト一つで、画像や動画中の任意の物体を高精度にマスク分割できる汎用性」** です。

### 1. プロンプト駆動の汎用セグメンテーション

- ユーザーが**点・ボックス・テキスト・マスク**などのプロンプトを与えると、  
  それに応じて**任意の物体をゼロショットでマスク分割**できます。
- 特定のカテゴリに限定されず、  
  - 未知の物体  
  - 複雑な形状  
  - 動画中のオブジェクト  
  にも対応できるのが最大の特徴です。

### 2. 動画対応と時間方向の一貫性

- 静止画だけでなく**動画**にも対応し、  
  フレーム間でオブジェクトを追跡しながら一貫したマスクを生成できます。
- これにより、動画ベースの応用（追跡、AR、ロボットビジョンなど）を支える基盤となります。

### 3. 高精度な境界情報

- **Hiera**という階層型ビジョントランスフォーマーをバックボーンに採用し、  
  細かい境界や複雑な形状（髪の毛、葉っぱ、網目など）も高精度にマスクできます。


## 汎用性を得た仕組み

SAM2が「どんな物体でもセグメントできる」という**汎用性**を得た仕組みは、主に以下の3つの要素で成り立っています。

### 1. 巨大で多様なデータでの事前学習

- SAM2は、**1,100万枚以上の画像**と**10億以上のマスク**という膨大なデータで事前学習されています[SAM2](https://arxiv.org/abs/2406.XXXXX)（正確な数値は論文版により異なりますが、初代SAMと同様の規模感）。
- このデータには、
  - 日常的な物体（人、車、動物など）
  - 珍しい物体（工具、楽器、特殊な形状など）
  - 複雑な背景や細かい境界（髪の毛、葉っぱ、網目など）
  が含まれており、**「ありとあらゆる物体とシーン」を見ている**状態です。
- これにより、モデルは**未知の物体でも輪郭や形状のパターンを学習**しており、ゼロショットで対応できるようになっています。

### 2. プロンプト駆動の設計（ユーザーが「どこ」を教える）

SAM2は、**「何をセグメントするか」をユーザーがプロンプトで指定する**設計になっています。

__プロンプトの種類__

- **点プロンプト**：  
  - 画像上の一点をクリック → 「この点が示す物体をセグメントして」
- **ボックスプロンプト**：  
  - 矩形で囲む → 「このボックス内の物体をマスクして」
- **テキストプロンプト**：  
  - 「猫」「車」などのテキスト → 「このテキストが示す物体をセグメントして」
- **マスクプロンプト**：  
  - 既存のマスクを入力 → 「このマスクを改善・拡張して」

__仕組み__

- プロンプトエンコーダが、点・ボックス・テキスト・マスクなどを**共通の特徴ベクトル**に変換します。
- 画像エンコーダ（Hiera）が画像全体の特徴を抽出します。
- マスクデコーダが、**プロンプト特徴と画像特徴を統合**し、「ユーザーが指定した場所・意味に対応するマスク」を生成します。

**ポイント**：  
モデル自身が「何が重要か」を決めるのではなく、**ユーザーがプロンプトで「ここが重要」と教える**ため、  
未知の物体でも「この点が示す物体」として正しくマスクできるようになります。

![1785704071081](image/3_sam2/1785704071081.png)

### 3. 強力なバックボーン（Hiera）とマルチスケール特徴

SAM2は、**Hiera**という階層型ビジョントランスフォーマーをバックボーンに採用しています。

__Hieraの特徴__

- **階層型アーキテクチャ**：  
  - 低解像度の特徴（物体全体の形）  
  - 高解像度の特徴（細かい境界やテクスチャ）  
  を段階的に統合します。
- **マルチスケール処理**：  
  - 大きな物体も小さな物体も、それぞれに適したスケールで特徴を捉えます。

__マスクデコーダの役割__

- プロンプト特徴と、Hieraが抽出した**マルチスケールの画像特徴**を組み合わせ、  
  - 物体の大まかな形  
  - 細かい境界  
  を両方考慮したマスクを生成します。

これにより、**複雑な形状や細かい境界も高精度にマスク**できるようになっています。

![1785790233319](image/3_sam2/1785790233319.png)

### 4. 動画対応の工夫（時間方向の一貫性）

動画版SAM2では、さらに以下の工夫が加えられています。

- **時間方向のアテンション**：  
  - フレーム間で特徴を関連付け、オブジェクトの動きや形状変化を追跡します。
- **トラッキング機構の統合**：  
  - セグメンテーションとトラッキングを同時に行い、  
    フレーム間でオブジェクトIDが変わらないようにします。

これにより、**動画全体で一貫したマスク**を生成できるようになっています。

## 実験

SAM2（Segment Anything Model 2）の論文[SAM2論文](https://arxiv.org/html/2408.00714v1)では、**Promptable Visual Segmentation (PVS)** というタスクを中心に、画像・動画の両方で広範な実験を行っています。

### 1. 問題設定：Promptable Visual Segmentation (PVS)

__タスク定義__
- **PVS（Promptable Visual Segmentation）**：  
  - 入力：  
    - 画像 or 動画  
    - 任意のフレームに対する**プロンプト**（点・ボックス・マスク）  
  - 出力：  
    - プロンプトが指し示す**対象オブジェクトの時空間マスク（masklet）**  
      - 画像：単一フレームのマスク  
      - 動画：フレーム間で一貫したマスク（オブジェクト追跡を含む）

__入力・出力__
- **入力**：  
  - 画像 or 動画（1フレームでも動画として扱う）  
  - プロンプト（点・ボックス・マスク）  
- **出力**：  
  - 対象オブジェクトの**マスク（0/1マップ）**  
  - 動画の場合：フレーム間で一貫したマスク（オブジェクトIDを維持）

__評価指標__
- **動画セグメンテーション**：  
  - **J&F**：Jaccard（IoU）と F-score の平均  
    - DAVIS, MOSE, YouTube-VOS などで標準的に使用  
- **画像セグメンテーション**：  
  - **mIoU**（mean Intersection over Union）  
  - 1-click mIoU（1回のクリックプロンプトでの平均IoU）  
- **速度**：  
  - **FPS（Frames Per Second）**：リアルタイム性の指標  
- **インタラクティブ性**：  
  - 目標精度に到達するまでの**ユーザー操作回数（interactions）**

### 2. 使用データセット

__訓練データ__
- **SA-V（Segment Anything Video）**：  
  - SAM2のために構築された**最大規模の動画セグメンテーションデータセット**  
  - 50.9K 動画、35.5M マスク  
  - モデル・イン・ザ・ループのデータエンジンで収集  
- **SA-1B（Segment Anything 1B）**：  
  - 初代SAMで使用された11M画像・1.1Bマスクの巨大データセット  
  - 画像セグメンテーションの事前学習に使用

__評価データ（ゼロショット）__
- **動画ベンチマーク（17種類）**：  
  - DAVIS 2016/2017  
  - YouTube-VOS  
  - MOSE  
  - LVOS v2  
  など、主要なVOS（Video Object Segmentation）データセット
- **画像ベンチマーク（37種類）**：  
  - COCO, ADE20K, Pascal VOC など  
  - 参照表現セグメンテーション、エッジ検出、インスタンスセグメンテーションなど多様なタスク

### 3. ベースラインとの比較

__動画セグメンテーション__
- **モジュール型ベースライン**：  
  - SAM（初代）＋ビデオトラッカー（XMem++, Cutie など）  
  - セグメンテーションとトラッキングを別々のモデルで行う従来手法
- **スタンドアロンVOS SOTAモデル**：  
  - DEVA, Cutie など、動画オブジェクトセグメンテーション専用モデル

__画像セグメンテーション__
- **SAM（初代）**：  
  - ViTバックボーン、画像専用  
  - ゼロショット性能と速度を比較対象

### 4. 主な実験結果

__(1) 動画セグメンテーション性能（ゼロショット）__

- **DAVIS 2017**：  
  - SAM2（Hiera-L）：**J&F 91.6**  
  - Cutie（SOTA VOSモデル）を**大きく上回る**性能[SAM2論文](https://arxiv.org/html/2408.00714v1)
- **SA-V test set**：  
  - SAM2（Hiera-L）：**J&F 77.6**  
  - モジュール型ベースライン（SAM＋トラッカー）よりも高精度
- **MOSE, LVOS v2** など17の動画ベンチマークで、  
  - 既存のVOSモデルやモジュール型手法を**一貫して上回る**ゼロショット性能

__(2) 画像セグメンテーション性能（ゼロショット）__

- **1-click mIoU**：  
  - SAM2：**58.9**  
  - SAM（初代）：58.1  
  - SAM2が**より少ない操作で高精度なマスク**を生成可能[SAM2論文](https://arxiv.org/html/2408.00714v1)
- **速度**：  
  - SAM2はSAM（初代）よりも**約6倍高速**（FPS比較）  
  - 同じ精度を達成するのに**3倍少ないインタラクション**で済む

__(3) インタラクティブ性能（ユーザー操作の効率）__

- **動画タスク**：  
  - モジュール型ベースライン（SAM＋トラッカー）に比べ、**3倍少ないインタラクション**で同等以上の精度を達成[SAM2論文](https://arxiv.org/html/2408.00714v1)
- **画像タスク**：  
  - 1クリックでのmIoUが向上し、  
    ユーザーが「どこをクリックするか」をあまり気にしなくても高精度なマスクが得られる

## 総括

SAM2の特徴を**仕組みと合わせて**簡潔にまとめると、以下のようになります。

### 1. プロンプト駆動の汎用セグメンテーション

- **特徴**：  
  点・ボックス・テキスト・マスクなどの**プロンプト一つで、任意の物体をゼロショットでマスク分割**できる。
- **仕組み**：  
  - プロンプトエンコーダが点・ボックス・テキスト・マスクを**共通の特徴ベクトル**に変換。  
  - 画像エンコーダ（Hiera）が画像全体の特徴を抽出。  
  - マスクデコーダが**プロンプト特徴と画像特徴を統合**し、「ユーザーが指定した場所・意味に対応するマスク」を生成。

### 2. 動画対応と時間方向の一貫性

- **特徴**：  
  静止画だけでなく**動画**にも対応し、フレーム間で一貫したマスクを生成できる。
- **仕組み**：  
  - 画像を「1フレームの動画」として統一的な入力に扱う。  
  - **ストリーミングメモリ**を持つトランスフォーマーアーキテクチャで、  
    フレーム間の特徴を関連付け、オブジェクトの動きや形状変化を追跡。

### 3. 高精度な境界情報（Hieraバックボーン）

- **特徴**：  
  細かい境界や複雑な形状（髪の毛、葉っぱ、網目など）も高精度にマスクできる。
- **仕組み**：  
  - **Hiera**という階層型ビジョントランスフォーマーをバックボーンに採用。  
  - 低解像度の特徴（物体全体の形）と高解像度の特徴（細かい境界）を段階的に統合。  
  - マスクデコーダがマルチスケール特徴とプロンプト特徴を組み合わせてマスクを生成。

### 4. 巨大データでの事前学習（SA-V＋SA-1B）

- **特徴**：  
  未知の物体や複雑なシーンでもゼロショットで対応できる汎用性。
- **仕組み**：  
  - **SA-V**（50.9K動画、35.5Mマスク）と**SA-1B**（11M画像、1.1Bマスク）という巨大データで事前学習。  
  - モデル・イン・ザ・ループのデータエンジンで、ユーザーインタラクションを通じてデータとモデルを継続的に改善。

### 5. 高速・高効率なインタラクティブ性能

- **特徴**：  
  SAM（初代）より**約6倍高速**で、**3倍少ない操作**で高精度なマスクを生成できる[SAM2論文](https://arxiv.org/html/2408.00714v1)。
- **仕組み**：  
  - Hieraバックボーンとストリーミングメモリによる効率的な推論。  
  - プロンプト駆動設計により、ユーザーが「どこを重視するか」を明確に指定できるため、  
    少ない操作で目的のマスクに到達できる。

### まとめ

SAM2は、

- **プロンプト駆動**で任意の物体をマスク  
- **動画対応**で時間方向の一貫性を確保  
- **Hieraバックボーン**で高精度な境界を実現  
- **巨大データでの事前学習**で汎用性を獲得  
- **高速・高効率なインタラクティブ性能**を実現  

という仕組みにより、**「画像・動画中の任意の物体を、プロンプト一つで高精度にマスク分割できる汎用基盤」** として設計されています[SAM2論文](https://arxiv.org/html/2408.00714v1)。

