...
def train_feature_alignment(model, dataloader, epochs=1, lr=1e-4):
    """
    第1段階：特徴整合（プロジェクタのみ学習）
    """
    accelerator = Accelerator()
    model.train()
    # プロジェクタのみ学習
    optimizer = optim.AdamW(model.image_proj.parameters(), lr=lr)
    ...
    for epoch in range(epochs):
        ...
        print(f"Feature Alignment Epoch {epoch+1}, Loss: {avg_loss:.4f}")


def train_instruction_tuning(model, dataloader, epochs=2, lr=2e-5):
    """
    第2段階：指示チューニング（言語モデル＋プロジェクタを微調整）
    """
    accelerator = Accelerator()
    model.train()
    # 言語モデル＋プロジェクタを学習（LoRA重みも含む）
    ...
    for epoch in range(epochs):
        ...
        for images, texts in dataloader:
            with accelerator.accumulate(model):
                ...
                total_loss += loss.item()
        avg_loss = total_loss / len(dataloader)
        print(f"Instruction Tuning Epoch {epoch+1}, Loss: {avg_loss:.4f}")
