...
Attentionを効率化・高速化するための工夫は、計算対象を絞る「Sparse（スパース）」以外にも多岐にわたります。

現在のLLM（大規模言語モデル）の進化を支えている、主要な3つのアプローチ（計算の効率化、構造の簡略化、並列性の向上）について解説します。

---

## 1. 入力トークン数を減らす「線形化・近似」

Attentionの計算量が **$O(N^2)$**（文章の長さの二乗）になるのを防ぐため、数学的に近似する手法です。

* Linear Attention (線形アテンション):
  Softmax計算の順序を数学的に入れ替えることで、計算量を $O(N)$（文章の長さに比例）にまで落とす手法です。代表例として RetNet や Linear Transformer があります。
* 低ランク近似 (Low-rank Approximation):
  巨大な行列を、情報の密度を保ったまま小さな行列の積で表現する手法です（例：Linformer）。

---

## 2. キーと値のセットを使い回す「共有化」

推論時のメモリ（KVキャッシュ）消費を劇的に抑えるための構造的な工夫です。

* Multi-Query Attention (MQA):
  複数のヘッドがそれぞれ独自の Query を持ちつつ、Key と Value は全員で1セットのみを共有する手法です。
* Grouped-Query Attention (GQA):
  MQAと通常のAttentionの中間で、ヘッドをいくつかのグループに分け、グループごとに1セットの Key/Value を共有します。
  * **効果:** Llama 3 や Mistral などの最新モデルで採用されており、精度を保ちつつ推論速度とメモリ効率を大幅に向上させています。

[Image comparing Multi-Head Attention vs Multi-Query Attention vs Grouped-Query Attention architectures]

---

## 3. 計算順序とメモリ管理の最適化

アルゴリズムの定義自体は変えず、コンピュータのハードウェア（GPU）の動きに合わせる工夫です。

* FlashAttention (前述の通り):
  タイリングと再計算により、メモリの読み書き速度（I/O）のボトルネックを解消します。
* PagedAttention:
  OSの「仮想メモリ」の考え方を導入し、KVキャッシュをバラバラのメモリ領域に効率よく配置します（vLLMなどで使用）。これにより、一度に処理できるユーザー（バッチサイズ）を数倍に増やせます。

---

## 4. そもそも「注意を向けない」状態を作る工夫

* Sliding Window Attention (SWA):
  特定の範囲（窓）の中だけを見る手法です。層を重ねることで、間接的に遠くの情報も伝わります（例：Mistral 7B）。
* Log-Sparse Attention:
  等間隔ではなく、指数関数的に離れた位置のトークンだけをサンプリングして見る手法です。

---

## まとめ：何が主流か？

現在、最も実用的な「最強の組み合わせ」は以下の通りです。

1. **学習時:** FlashAttention-2 (計算効率の極致)
2. **推論時:** Grouped-Query Attention (構造の工夫) + PagedAttention (メモリ管理)
3. **長文対応:** RoPE (相対位置エンコーディング) と SWA (窓アテンション) の組み合わせ

### 次のステップへの提案

もしあなたが **「モデルの推論を速くしたい（例えばローカルで動かしたい）」** のであれば、構造を変える **GQA** やメモリを管理する **PagedAttention** の仕組みをさらに深掘りするのがおすすめです。

一方、**「モデルを自分で学習させたい」** のであれば、**FlashAttention** をいかに使いこなすかが重要になります。どちらの方向に興味がありますか？
